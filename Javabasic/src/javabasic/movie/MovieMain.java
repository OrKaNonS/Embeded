...
//		Dao 예약 코드
		...
		String userId = "JD123";
		 ...
		 // 좌석을 예약하기 전에 해당 좌석이 이미 예약되었는지 확인합니다
        boolean alreadyReserved = md.isReserved(seatRow, seatNum);
        if (alreadyReserved) {
            System.out.println("해당 좌석은 이미 예약되었습니다.");
        } else {
		// 좌석을 예약합니다
		boolean isReserved = md.reserveSeat(seatRow, seatNum, userName, userId);
		if (isReserved) {
